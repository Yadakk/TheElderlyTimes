...
namespace Verpha.HierarchyDesigner
{
    public static class HierarchyDesigner_Manager_GameObject
    {
        ...
        private static Dictionary<int, Texture2D> backgroundTextureCache = new Dictionary<int, Texture2D>();
        ...
        {
            if (!backgroundTextureCache.TryGetValue(instanceID, out Texture2D backgroundTexture))
            {
                backgroundTexture = defaultTexture;
                backgroundTextureCache[instanceID] = backgroundTexture;
            }
        ...
        private static void DrawFolder(GameObject gameObject, Rect selectionRect, int instanceID)
        {
            ...
            GUIStyle textStyle = new GUIStyle
            {
                ...
                fontSize = folderInfo.fontSize,
                fontStyle = folderInfo.fontStyle,
                normal = { textColor = gameObject.activeInHierarchy ? folderInfo.textColor : ConvertActiveColorToInactive(folderInfo.textColor, 0.5f) }
            };

            Rect labelRect = new Rect(selectionRect.x + 18, selectionRect.y, selectionRect.width - 18, selectionRect.height);
            GUI.Label(labelRect, gameObject.name, textStyle);
        }
